...
struct ServerUpdateInfo: Identifiable, Sendable {
    ...
    let changelog: ServerChangelogEntry?
}
...
final class ServerUpdateChecker {
    ...
    private func fetchUpdateInfo(from apiClient: APIClient) async throws -> ServerUpdateInfo? {
        ...
        // 2. Fetch changelog for the new version
        let changelog = try? await fetchChangelog(version: latestVersion, from: apiClient)
    ...
    private func fetchChangelog(version: String, from apiClient: APIClient) async throws -> ServerChangelogEntry? {
        let request = try apiClient.network.buildRequest(
            path: "/api/changelog",
            authenticated: true,
            timeout: 10
        ...
        guard let raw = allEntries[version] else { return nil }

        return ServerChangelogEntry(
            version: version,
            date: raw.date ?? "",
            added: raw.added ?? [],
            fixed: raw.fixed ?? [],
